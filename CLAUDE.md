...
- An npm CLI (`@maccesar/aiskills`) + Claude Code plugin marketplace that ships 11 general-purpose AI coding assistant skills (audit-codebase, humaniza, laravel-security-sweep, npm-supply-chain, refactoring-ui, release, seo-launch, session-log, stitch-showcase, technical-demo-videos, vscode-extension-dev). `release` is explicit-only because it can commit, tag, push, and publish.
...
**The maintainer does not use the marketplace channel — do not recommend refreshing it after a release.** Both `@maccesar/aiskills` and `@maccesar/titools` are `npm link`-ed to their checkouts, `~/.agents/skills/<skill>` are symlinks into `skills/`, and there is no `maccesar-*` entry in `~/.claude/plugins/cache/` (verified 2026-09-14). Edits are live the moment they are saved; the only step the maintainer runs is `aiskills update`, which adds symlinks for new skills. The marketplace facts above are for end users. Precedent: after v1.25.0 and v1.26.0 the closing message told him to run `/plugin marketplace update` → `aiskills install` → `/reload-plugins`, copied from the old version of this line; his answer: *«SIEMPRE me recomiendas actualizar el SKILL a través del Market Place.. PERO sabes perfectamente que tengo `npm link`»*.
