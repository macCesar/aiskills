...
The sweep now runs 18 line patterns and 7 project-level checks.
...
The patterns are the ones behind vulnerabilities a multi-agent scan confirmed in a production Laravel 10 application: a role middleware that let unregistered routes through, a public mobile registration writing into the panel's `users` table, raw `MATCH ... AGAINST` queries escaped with `e()`, reset links built from the `Host` header, SSRF in site monitoring, client-controlled upload directories and names, and a signing secret with a literal fallback. Run against that application's pre-fix snapshot, the script flagged every confirmed location; against the fixed tree, the SQL, reset, secret, host-header and image-size matches dropped to zero.

`composer audit` runs with `--no-plugins`, and output that is not the expected JSON is reported as an error rather than as a clean result: during validation, a Laravel 8 project whose `composer.lock` predates `allow-plugins` made the command abort with empty output, which an early version of the script reported as "no advisories" for a project with 83. Laravel 3 and 4 are reported as out of scope rather than swept, since their structure would make the patterns report little and imply much. Version-specific notes were checked against the Laravel 13.x documentation (the `PreventRequestForgery` rename, `trustHosts(at:)`, the `extensions` rule) and against local Laravel 8, 9 and 10 projects (`$routeMiddleware` versus `$middlewareAliases`).
...
### Fixed — marketplace diagnostics and sibling API parity
...
- **New skill: `npm-supply-chain`** — the two halves of the registry, both rewritten between November 2025 and July 2026: how a package gets published, and what runs on the machine installing one. Eighth skill in the set; `lib/config.js` `SKILLS`, both plugin manifests, the README table and the skill-count line in `AGENTS.md`, `CLAUDE.md` and `docs/project/context.md` were updated to match.
  - **Why a skill and not more `/release`.** The knowledge is dated, specific to npm, and consumed once per project: migrating to trusted publishing happens a single time and then never again. `/release` is 415 lines that load on **every** release and deliberately spans npm, Titanium, Composer, Cargo and CocoaPods, so putting an npm doctrine inside it would charge every release of every project type for something that already happened. What `/release` got instead is a pointer, not the body — see below.
  - **Five reference files.** `authentication.md` (the dated timeline, the two-hour session that replaced classic tokens, the three token types, the 2FA-bypass retirement in its two phases — 31 July 2026 applied, ~January 2027 pending — and what it explicitly does not touch: GitHub PATs, App tokens and `GITHUB_TOKEN`), `trusted-publishing.md` (the publisher form field by field, `id-token: write`, Node ≥ 22.14.0 / npm ≥ 11.5.1, why `NODE_AUTH_TOKEN` must be absent, and the version guard), `install-defaults.md` (npm v12: `allowScripts` off, `--allow-git`, `--allow-remote`, `npm approve-scripts --allow-scripts-pending` and the committed allowlist), `migration.md` (the once-per-project order of operations, including what only the package owner can do with their 2FA), and `verification.md` (the command behind each claim).
  - **`scripts/auditar_npm.py`** measures the repo instead of interviewing the user. Standard library only, writes nothing. It reports token lines in `~/.npmrc` and the project `.npmrc`, Actions secrets that look like npm credentials **and whether any workflow still references them**, how each publishing workflow authenticates, version files that drifted apart, shields.io badges pointing at a package that does not exist, dependencies that would prompt for approval on npm v12, and the local npm version. It reads secret *names* only — a value is never printed, because a token that lands in a transcript is a token that has to be revoked.
  - **The auditor was validated against a positive and a negative control**, not just run once: green on this repo and on TiTools, both of which are correctly configured, and on a fixture carrying a token-auth workflow, drifted version files, a badge with the wrong package name and a dependency with an install script it reported all four. An instrument that says the same thing about every repo is not measuring anything, which is the lesson from `run_eval.py` giving 0/10 to everything.
  - **`test/npm-supply-chain.test.js`** encodes those two controls as fixtures rather than asserting on the script's source, plus `py_compile`, the CLI contract, a guard that the script can never read a secret's value, and YAML validation of the shipped template. One test exists for a defect found while writing it: the template's own comments say "no NPM_TOKEN", and the first version of the credential scan read that sentence as a credential and reported the correct file as the broken one. Comments are now stripped before any workflow is classified.
  - **`assets/publish.yml`** is the workflow template, commented line by line — the same file already running in this repo, generalized.

### Changed

- **`/release` now bumps secondary version files.** The command never mentioned `.claude-plugin/plugin.json`, while this repo's own release checklist requires it: Claude Code compares that field to invalidate its plugin cache, so a repo shipping to npm and to a plugin marketplace could publish new code while marketplace users kept running the old one. It was correct here only because it was done by hand; anyone else running `/release` in a repo with a plugin manifest hit the bug. Step 1 detects it, Step 4 shows both bumps in the plan, and Phase 2 applies them in the same commit.
- **`/release` says when the tag publishes.** It now reads `.github/workflows/` and, if a workflow triggers on a `v*` tag and publishes, states in the confirmation block that pushing the tag will ship the package — previously the user confirmed what looked like a labelling step and got a publication. Phase 5 then waits for that run and reports whether it went green, since the release is not finished when `git push` returns: the run can still fail on the version guard, on tests, or on a credential that was never configured.
- **The citation contract explains itself instead of shouting.** The three documentation-grounded skills — `refactoring-ui`, `vscode-extension-dev` and `npm-supply-chain` — opened their output contract with an all-caps `MUST`, the only capitalized imperative left in the repo. It now states the reason in one shared paragraph: a cited value and a remembered one look identical once written, so the citation is the only thing that distinguishes them, and collecting sources at the end means reconstructing where something came from rather than recording it. The wording is byte-identical across the three, as the template in `CLAUDE.md` requires, and that template now carries the reason too so the next skill copies the reasoned version. An instruction whose purpose the model understands survives cases the rule never anticipated; a bare imperative does not.

- **`/release` recommends trusted publishing when there is none.** On an npm project with no publishing workflow, the plan carries one line pointing at the `npm-supply-chain` skill. It recommends and does not act: that migration needs the package owner's interactive 2FA on npmjs.com and has no business starting in the middle of a release awaiting confirmation.

### Fixed

- **The repo's own description of the skill spec was wrong, and the test enforced the wrong rule.** `AGENTS.md` listed the allowed frontmatter fields as `name`, `description`, `metadata.*`, `argument-hint` and `allowed-tools` — checked against [the specification](https://agentskills.io/specification), that omits two real fields (`license` and `compatibility`) and blesses one that is not a skill field at all (`argument-hint` is Claude Code slash-command frontmatter and belongs in `commands/*.md`). The rule dates to May 2026 and travelled here from TiTools unverified. Corrected to the six fields the spec actually defines.
  - **The 1024-character cap belongs to `description`, not to the frontmatter block.** `test/manifest.test.js` measured the whole block, which is stricter than the spec by however long the other fields happen to be — strict enough that adding a legal `compatibility` line to a skill near the limit would have failed the suite. The check now measures the description, and a second one enforces the `name` rules the spec states and nothing was checking: 64 characters, lowercase alphanumerics and single hyphens.
  - **`vscode-extension-dev` gets back the 160 characters the wrong rule cost it.** v1.17.0 trimmed its description from 1009 to 849 characters for crossing a block limit that does not exist — at 1009 it was comfortably inside the real cap. The trim deleted six distinct triggers (`designing`, `commands`, `configuration`, `progress indicators`, `FileSystemWatcher`, `Diagnostics`, plus `webpack`, Claude Code `skills`, and the parenthetical listing what "configuring your own editor" means), and the commit that made it claimed "no distinct trigger was removed" — a claim that was wrong: someone asking about a `FileSystemWatcher` is not caught by `TreeView`. Restored, keeping the better wording that release introduced, at 1009/1024.
  - **A sweep of every description's history found exactly one casualty.** `stitch-showcase` also dropped sharply (852 → 560) but in a May 2026 docs commit, three months before the limit test existed, and it has since grown back. In TiTools the drops are larger and older — its eight skills went from ~1000 to ~300 characters in January 2026 — but those were two deliberate triggering rewrites, not limit trims. Reference files were never at risk: none of them carries frontmatter.
  - **The same three defects are fixed in TiTools**, which carries the identical test and five copies of the wrong rule in its `AGENTS.md`. Its own descriptions all sit well under the cap today, so nothing there needed restoring.
  - **The four skills that ship scripts now declare `compatibility`**, which is what the field is for: `humaniza`, `seo-launch`, `stitch-showcase` and `npm-supply-chain` all need Python 3, all standard library, and `seo-launch` additionally needs network access and ImageMagick for stage 2. Nothing about the skills changed — this is the requirement being stated where an agent can read it before running something that fails.

- **The README documented a skill frontmatter that no skill uses.** It showed `when_to_use`, `source` and `anti_hallucination_note`, none of which are in the agentskills.io specification and none of which appear in any `SKILL.md` here — `AGENTS.md` explicitly forbids fields outside the spec. Following the README produced a skill that fails the 1024-character frontmatter test and buries its triggers in fields no assistant reads. Replaced with the shape actually in use, plus why the `description` is the only place triggers work.
- **The skill count said 6.** `CLAUDE.md` line 120 was left behind when `seo-launch` shipped, while three other lines in the same repo said 7. All four now say 8.
...
- **An ESLint flat config, so `npm run lint` runs again.** The `lint` script has been in `package.json` all along, but there was no configuration file — ESLint 9 dropped `.eslintrc` support, so every invocation died on "couldn't find an eslint.config.js" and the script had been dead ever since. `eslint.config.js` covers `bin/`, `lib/` and `test/` with the recommended rule set; `skills/` is excluded, since it is Markdown, Python and template fragments. An argument named with a leading underscore is exempt from `no-unused-vars`, because that name is how a signature documents a parameter the caller must still pass. Turning it on surfaced four pieces of dead code, removed here: the unused `getAgentsSkillsDir` and `selectCancel` imports in `lib/commands/skills.js`, a `skillsResult` assigned from `installSkills()` and never read, and an unreferenced `check()` helper in `lib/prompts/checkboxCancel.js`.
  - **`@eslint/js` and `globals` join devDependencies**, both pinned to the ESLint 9 line: npm resolves `@eslint/js` to 10 by default, which requires ESLint 10 and fails the install outright.
  - **The script itself was also skipping ten files.** npm runs scripts through `sh`, where `**` behaves as `*`, so `eslint lib/**/*.js` expanded to `lib/*/*.js` and never looked at anything directly under `lib/` — `config.js`, `installer.js`, `symlink.js`, `downloader.js` and six more. It is now `eslint bin lib test`, which does not depend on the shell. `"format"` carries the same defect and was left alone on purpose: correcting it would reformat ten files that never passed through Prettier, and that diff is a decision to take deliberately, not a side effect of fixing the linter.

- **`.github/workflows/publish.yml` publishes the package from GitHub Actions using trusted publishing (OIDC).** It runs on a pushed `v*` tag, which is the last thing `/release` does, so the release flow ends where it already ended and the publish happens on its own. There is no `NPM_TOKEN` and no secret of any kind: `permissions: id-token: write` lets the runner mint a short-lived OIDC credential that npm verifies against the publisher registered for this package, and npm attaches provenance to the publish automatically as a result.
  ...
  - **The job refuses to publish a tag that disagrees with the version files.** It compares the tag against `package.json` and `.claude-plugin/plugin.json` and exits before `npm ci` if any of the three differ. This repo has never hit that, but TiTools shipped 2.6.0 to npm while its `plugin.json` still read 3.0.0 and the marketplace announced the wrong number; the check costs one step and removes the whole class of mistake.
  - **The trusted publisher itself is registered on npmjs.com**, per package, under Settings → Trusted Publisher, and it names this workflow by filename. Renaming this file breaks publishing until the registration is updated to match.
...
- **`aiskills list` wraps descriptions to two lines instead of truncating them to one.** The lines align under the description column and size themselves to the terminal width, with a 24-column floor so a narrow window still says something. Two lines is the cap on purpose: past that the text is trigger phrasing written for the agent, not for this screen.
  - **Every row was printing a stray leading `'`.** The parser matched `description: "…"` but not the single-quoted YAML that all seven skills use — the descriptions contain colons and apostrophes, which is why they are quoted that way — so it fell through to a bare `(.+)` branch that captured the opening quote as part of the text. The same branch stopped at the first newline, so a description written across two lines would have been cut mid-sentence; none is today, but the parser no longer depends on that. Unquoting also resolves the doubled apostrophe that single-quoted YAML uses as an escape, which `audit-codebase` has in its description.
  - `wrapDescription()` and `parseDescription()` are exported and covered by the new `test/list.test.js`, which is where the two behaviors above are pinned, along with the wrapper's edges: a word wider than the column, a window narrower than the floor, and a description spanning two lines.
