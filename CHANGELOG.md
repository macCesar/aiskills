...
- **`npm run lint` had never run.** The repo shipped no ESLint configuration at all, so since ESLint 9 dropped `.eslintrc` support every invocation died with "couldn't find an eslint.config.js" — the script was decorative. `eslint.config.js` is new (flat config: `js.configs.recommended`, Node globals, `skills/` ignored), with `@eslint/js` and `globals` added as devDependencies, both pinned to the ESLint 9 line because `@eslint/js@10` requires ESLint 10 and the resolution fails otherwise.
- **The lint script silently skipped ten files.** npm runs scripts through `sh`, where `**` behaves as `*`, so `eslint lib/**/*.js` expanded to `lib/*/*.js` and never looked at anything directly under `lib/` — `config.js`, `installer.js`, `symlink.js`, `downloader.js` and six more. It is now `eslint bin lib test`, which does not depend on the shell and also covers the CLI entrypoint and the suite. `"format"` carries the same defect and was deliberately left alone: fixing it would reformat ten files that never passed through Prettier, and that diff is a decision, not a side effect.
